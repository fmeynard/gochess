...
- `internal/board/position-updater_test.go` now contains explicit regression coverage for promotion undo with and without capture.
...
- `internal/engine/engine_legal_moves_test.go` now checks that legal moves never capture the enemy king.
- `internal/engine/engine_perft_test.go` contains a dedicated regression for the promotion/perft case.
...
- If a benchmark-only optimization is considered, document it clearly in `docs/benchmark-history.md` and keep it isolated.
