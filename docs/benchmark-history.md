...
- split the engine internals cleanly into:
  - `internal/board`
  - `internal/movegen`
  - `internal/engine`
- preserved hot-path performance after the package refactor by replacing dynamic `kingAffectMask(...)` recomputation with a precomputed table
...
Perft tricks: false
Nodes: 178633661
Elapsed: 5.650972876s
CPU profile: .codex-tmp/bench-perft.cpu.prof
```

Interpretation:

- the `board` / `movegen` / `engine` split itself did not need to cost throughput
- the main post-refactor regression was updater-local and was recovered by precomputing king-affect masks
- the current reference remains effectively in the same band as `v18`
