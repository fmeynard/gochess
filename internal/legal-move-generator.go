...
func appendNormalMovesFromMask(piece Piece, startIdx int8, targets uint64, dst []Move, count int) int {
	for targets != 0 {
		...
		targets &^= 1 << targetIdx
		dst[count] = Move{piece: piece, startIdx: startIdx, endIdx: targetIdx, flag: NormalMove}
		count++
	}
...
func appendCaptureMovesFromMask(piece Piece, startIdx int8, targets uint64, dst []Move, count int) int {
	for targets != 0 {
		targetIdx := int8(bits.TrailingZeros64(targets))
		targets &^= 1 << targetIdx
		dst[count] = Move{piece: piece, startIdx: startIdx, endIdx: targetIdx, flag: Capture}
		count++
	}
...
func (e *Engine) appendNonKingMovesNoCheckNoPins(pos *Position, piecesMask uint64, friendlyOcc uint64, enemyOccNoKing uint64, enemyKingMask uint64, dst []Move, count int) int {
	for piecesMask != 0 {
		...
		switch piece.Type() {
		case Pawn:
			count = e.appendPawnMoves(pos, piece, idx, enemyOccNoKing, nil, 0, false, pos.activeColor, dst, count)
		case Knight:
			targets := knightAttacksMask[idx] &^ friendlyOcc &^ enemyKingMask
			count = appendNormalMovesFromMask(piece, idx, targets&^enemyOccNoKing, dst, count)
			count = appendCaptureMovesFromMask(piece, idx, targets&enemyOccNoKing, dst, count)
