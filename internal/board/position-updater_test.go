...
func TestPositionUpdater_UnMakeMove(t *testing.T) {
	...
	t.Run("Test Position 4", func(t *testing.T) {
		...
		legalMoves := []Move{
			NewMove(Piece(White|Bishop), B4, C5, NormalMove),
			NewMove(Piece(White|Pawn), C4, C5, NormalMove),
			NewMove(Piece(White|Pawn), D2, D4, PawnDoubleMove),
			NewMove(Piece(White|Rook), F1, F2, NormalMove),
			NewMove(Piece(White|Knight), F3, D4, NormalMove),
			NewMove(Piece(White|King), G1, H1, NormalMove),
		}
